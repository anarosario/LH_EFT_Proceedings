...
\caption{Parametrisation of the $gg\to H$ bins with no jet, 0-jet and 1-jet selection of the STXS as defined in its stage 1.2 with the parameters definitions of the \SMEFTatNLO\ model. The numbers are rounded according to their statistical uncertainty.}
\label{tab:higgseft:stxsggh1}
\end{table}
...
%and compare to study the $gg\to H$ and $gg\to ZH$ loop-induced processes. For the former, which is mainly meant for performing LO calculations, the $gg\to ZH$ process cannot be simulated and only one-loop functions for $gg\to H$ are implemented.
